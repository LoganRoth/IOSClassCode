...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let defEpiInfo = CurrentEpisodeInfo(epName: epiname, description: epidesc, season: 1, episode: 1)
        
        let allTrailers = generateTrailers(3)
        
        let moreLikeThis = exampleMovies
        
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, categories: categories, year: year, rating: "PG-13", numSeasons: numSeasons, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creators: creators, cast: cast, moreLikeThisMovies: moreLikeThis, episodes: allEpisodes, promoHeadline: headline, trailers: allTrailers)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImgURLString: thumbnail, description: desc, length: randLength, videoURL: exampleVideoURL)
